...
|---|---|---|
| ![Azure portal — App registration svc-foundry-dataplane-access-dev, Essentials: Application (client) ID <foundry-access-app-client-id>, Object ID, Directory (tenant) ID <tenant-id>, Client credentials "0 certificate, 1 secret", State Activated.](images/01-registered-app-foundry-dataplane.png) | ![Azure portal — Enterprise Application (Service Principal) svc-foundry-dataplane-access-dev, Properties: Name, Application ID <foundry-access-app-client-id>, Object ID <object-id>.](images/02-foundry-dataplane-service-principal.png) | ![Azure portal — the <project-name> Foundry project, Access control (IAM) → Role assignments filtered by the app; under "Foundry Agent Consumer (1)" the service principal svc-foundry-dataplane-access-dev is listed.](images/03-foundry-agent-consumer-role-assignment.png) |
...
APP_OBO_TENANT_ID=<tenant-id>
APP_OBO_CLIENT_ID=<obo-app-client-id>
...
![Azure portal — App registration svc-foundry-dataplane-access-dev, Essentials. The Application (client) ID (<foundry-access-app-client-id>) is highlighted, alongside the Object ID and the Directory (tenant) ID.](images/10-assign-foundry-user-sp-app-registration.png)

![Azure portal — Enterprise Application svc-foundry-dataplane-access-dev (Service Principal), Properties: Name, Application ID (<foundry-access-app-client-id>), and Object ID.](images/11-assign-foundry-user-sp-enterprise-app.png)

![Azure portal — the <project-name> Foundry project, Access control (IAM) → Role assignments filtered by the client ID; under "Foundry Agent Consumer (1)" the service principal svc-foundry-dataplane-access-dev is listed.](images/03-foundry-agent-consumer-role-assignment.png)
...
![Azure portal — Key Vault <your-keyvault>, Access control (IAM) → Add role assignment. Selected role: Key Vault Secrets User; "Assign access to: User, group, or service principal"; the Select members panel filtered by the agent identity's object ID shows the agent's service identity (cog-…-<project-name>-hello-world-python-…).](images/13-keyvault-add-role-assignment.png)

![Azure portal — Key Vault <your-keyvault>, Access control (IAM), role assignments grouped by role. Under "Key Vault Secrets User" the agent's service principal (<foundry-account>-<project-name>-hell…) is listed with its object ID.](images/14-keyvault-iam-agent-identity.png)
...
2026-07-06 00:22:51,592 INFO azure.ai.agentserver: Connectivity: project_endpoint=https://<foundry-account>.services.ai.azure.com
...
1. **Duplicate `.env` into `.env.docker`** and add the last 3 lines, so Docker can authenticate to Foundry with a **service principal** that is a *Foundry Agent Consumer* of that project (or agent). **These label names are fixed** (the names `DefaultAzureCredential` looks for):
   ...
   AZURE_CLIENT_ID=<foundry-access-app-client-id>
...
FOUNDRY_PROJECT_ENDPOINT=https://<foundry-account>.services.ai.azure.com/api/projects/<project-name>
AZURE_AI_PROJECT_ID=/subscriptions/<subscription-id>/resourceGroups/<resource-group>/providers/Microsoft.CognitiveServices/accounts/<foundry-account>/projects/<project-name>
...
```console
user@devbox:~/git_repos/hosted_agents/agents/hello-world-responses01$ azd provision
...
https://portal.azure.com/#@/resource/subscriptions/<subscription-id>/resourceGroups/<resource-group>/overview
```
...
```console
user@devbox:~/git_repos/hosted_agents/agents/hello-world-responses01$ azd deploy

Service                         Status        Duration
──────────────────────────────  ────────────  ──────────
● hello-world-python-responses  Done          1m25s

- Agent playground (portal): https://ai.azure.com/nextgen/r/<workspace-id>,<resource-group>,,<foundry-account>,<project-name>/build/agents/hello-world-python-responses/build?version=1
- Agent endpoint (responses): https://<foundry-account>.services.ai.azure.com/api/projects/<project-name>/agents/hello-world-python-responses/endpoint/protocols/openai/responses?api-version=v1
...
AGENT_HELLO_WORLD_PYTHON_RESPONSES_ENDPOINT="https://<foundry-account>.services.ai.azure.com/api/projects/<project-name>/agents/hello-world-python-responses/versions/1"
AGENT_HELLO_WORLD_PYTHON_RESPONSES_NAME="hello-world-python-responses"
AGENT_HELLO_WORLD_PYTHON_RESPONSES_RESPONSES_ENDPOINT="https://<foundry-account>.services.ai.azure.com/api/projects/<project-name>/agents/hello-world-python-responses/endpoint/protocols/openai/responses?api-version=v1"
...
![Microsoft Foundry portal — agent hello-world-python-responses (Running, Version 1), Details tab → Agent configuration. Under "Identity & access", the "Entra agent identity" ID (<agent-identity-object-id>) is highlighted with a copy button, alongside the "Entra agent blueprint" ID.](images/22-deployed-agent-identity-details.png)
...
![Azure portal — Key Vault <your-keyvault>, Add role assignment. Selected role: Key Vault Secrets User; "Assign access to: User, group, or service principal"; the Select members panel is filtered by the agent identity (<agent-identity-object-id>) and its ServiceIdentity (<agent-service-identity>) is selected, ready to click Select.](images/23-keyvault-grant-deployed-agent.png)
